  ...
  if (m_trxState == LORAWAN_PHY_RX_ON && !m_setTRXState.IsRunning ())
    {
      ...
      *interferenceAndNoise += *m_noise;
      ...
      if (sinr_db > sinr_cutoff_db)
        ...
        {
          NS_LOG_DEBUG (this << " packet dropped, sinr too low");
      ...
      if (m_errorModel != 0)
        {
          ...
          *interferenceAndNoise += *m_noise;
          ...
          if (m_random->GetValue () < per)
            {
              // this is where the packet gets destroyed... was there another packet being transmitted at the same time?
              //std::cout << "packet destroyed here, per was " << per << std::endl;
      ...
      if (!m_currentRxPacket.second.destroyed && !m_currentRxPacket.second.aborted)
      ...
      else
        {
          ...
          if (m_currentRxPacket.second.destroyed) {
            NS_LOG_DEBUG (this << " packet dropped, packet was destroyed");
            ...
            NS_LOG_DEBUG (this << " packet dropped, packet was aborted");
  ...
  if (m_trxState == LORAWAN_PHY_TX_ON)
            ...
            || (m_trxState == LORAWAN_PHY_BUSY_TX ) )
    {
      ...
      // Drop packet, hit PhyTxDrop trace
        NS_LOG_DEBUG ("Packet dropped, trxState is not TX ON");
      m_phyTxDropTrace (p);
      return;
    }
  else

...
LoRaWANFrameHeaderDownlink::Serialize (Buffer::Iterator start) const
{
  ...
  if (m_serializeFramePort) { //if framePort is 0, then the MAC layer messages are read directly from the payload. Else they are read from here (from FOpts)
    ...
    if(m_macCommandsNS[TimeSlotDelayReq].m_isBeingUsed) {
      NS_LOG_FUNCTION (this << "in TimeSlotDelay serialization");
      i.WriteU8 (TimeSlotDelayReq);
      i.WriteU8 (m_timeslotByte);
    } 
...
LoRaWANFrameHeaderDownlink::Deserialize (Buffer::Iterator start)
{
  ...
  while(fOptsLen > 0) {
    ...
    else if(command_id == TimeSlotDelayReq) {
      NS_LOG_FUNCTION (this << "in TimeSlotDelay deserialization");
      m_macCommandsNS[TimeSlotDelayReq].m_isBeingUsed = true;
      m_timeslotByte = i.ReadU8();
      fOptsLen--;
      nBytes++;
    }
    else {
...
bool LoRaWANFrameHeaderDownlink::AddLoRaTimeSlotDelayReq(uint8_t timeslot, uint8_t dataRateIndex) {

  //ensure mac command will fit
  uint8_t curr_len = m_frameControl & LORAWAN_FHDR_FOPTSLEN_MASK;
  if(curr_len + m_macCommandsNS[TimeSlotDelayReq].m_size > LORAWAN_FHDR_FOPTSLEN_MAX_SIZE) {
    //TODO: print err
    return false;
  }

  //ensure timeslot doesn't cause a delay of more than 10 seconds in total (dataRate dependent)
  if(timeslot > m_maxTimeSlotPushPerDataRate[dataRateIndex]) {
    return false;
  }

  m_frameControl += m_macCommandsNS[LinkADRReq].m_size; //add size of MAC command to the FOptsLen, which is [3:0] of FCtrl

  m_timeslotByte = timeslot;
  m_macCommandsNS[TimeSlotDelayReq].m_isBeingUsed = true;
  return true;
}

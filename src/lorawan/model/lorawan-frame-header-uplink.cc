...
LoRaWANFrameHeaderUplink::Serialize (Buffer::Iterator start) const
{
  ...
  if (m_serializeFramePort) { //if framePort is 0, then the MAC layer messages are read directly from the payload. Else they are read from here (from FOpts)
    ...
    if(m_macCommandsED[TimeSlotDelayAns].m_isBeingUsed) {
      i.WriteU8 (TimeSlotDelayAns);
      i.WriteU8 (m_timeslotStatus);
    }
...
LoRaWANFrameHeaderUplink::Deserialize (Buffer::Iterator start)
{
  ...
  while(fOptsLen > 0) {
    ...
    else if(command_id == TimeSlotDelayAns) {
      m_macCommandsED[TimeSlotDelayAns].m_isBeingUsed = true;
      m_timeslotStatus = i.ReadU8();
      fOptsLen--;
      nBytes++;
    }
    else {
...
bool LoRaWANFrameHeaderUplink::AddLoRaTimeSlotDelayAns(bool timeslotAck) {

  //check MAC command will fit into packet
  uint8_t curr_len = m_frameControl & LORAWAN_FHDR_FOPTSLEN_MASK;
  if(curr_len + m_macCommandsED[TimeSlotDelayAns].m_size > LORAWAN_FHDR_FOPTSLEN_MAX_SIZE) {
    return false;
  }
